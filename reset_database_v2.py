...
try:
    ...
    if response.lower() == 'yes':
        ...
        # Leer y ejecutar v001 con encoding UTF-8
        with codecs.open('database/migrations/versions/v001_initial_schema.sql', 'r', encoding='utf-8') as f:
            sql_content = f.read()
            cursor.execute(sql_content)
        print("   ✅ v001_initial_schema.sql ejecutado")
        
        # Leer y ejecutar v002 con encoding UTF-8
        with codecs.open('database/migrations/versions/v002_analytics_views.sql', 'r', encoding='utf-8') as f:
            sql_content = f.read()
            cursor.execute(sql_content)
        print("   ✅ v002_analytics_views.sql ejecutado")
        ...
        print("=" * 80)
        print("\n✨ Próximos pasos:")
        print("   1. El bot guardará las operaciones en 'trades'")
        print("   2. Los snapshots de cuenta en 'account_snapshots'")
        print("   3. La sesión actual en 'trading_sessions'")
        print("   4. Las vistas están listas para el dashboard")
    else:
        print("\n❌ Operación cancelada")
